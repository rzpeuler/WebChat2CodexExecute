...
const writingBlockTemplates = {
  LUNA_TASK: {
    ...
    scope: ['<填写允许修改的项目相对路径；目录请使用 path/**，文件请使用 path/to/file.ext>'],
    out_of_scope: ['<填写明确排除的范围；目录请使用 path/**，文件请使用 path/to/file.ext>'],
    deliverables: ['<填写交付物>'],
  ...
  GOVERNANCE_CHANGE: {
    schema_version: WRITING_BLOCK_TEMPLATE_VERSION,
    change_id: '<填写治理变更 ID>',
    operation: `<从 ${GOVERNANCE_CHANGE_OPERATIONS.join('、')} 中选择>`,
    document_id: '<填写文档 ID>',
    path: '<填写 docs/governance 下的相对路径>',
    reason: '<填写变更原因>',
    risk_level: `<从 ${GOVERNANCE_CHANGE_RISK_LEVELS.join('、')} 中选择>`,
