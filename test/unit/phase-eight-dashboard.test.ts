...
describe('phase eight dashboard contract', () => {
  ...
  it('keeps the fixed node contract and sanitizes graph input', () => {
    ...
    const lunaNode = snapshot.loopGraph.nodes.find((node) => node.id === 'run-luna');
  ...
  it('keeps currentNodeId only for an existing active node and rejects normalized dates', () => {
    ...
    expect(snapshot.loopGraph.nodes.find((node) => node.id === 'run-luna')?.updatedAt).toBe('1970-01-01T00:00:00.000Z');
  ...
  it('stops inspecting oversized node and detail arrays at hard limits', () => {
    ...
    expect(oversizedNodeSnapshot.loopGraph.nodes.find((node) => node.id === 'run-luna')?.state).toBe('PENDING');
    ...
    expect(oversizedDetailSnapshot.loopGraph.nodes.find((node) => node.id === 'run-luna')?.details).toHaveLength(
      LOOP_GRAPH_MAX_DETAILS,
    );
  });
  ...
  it('requires confirmation for manual Git maintenance commands', () => {
    ...
    expect(validateDashboardCommand({ command: 'force-consume-sol-output', confirm: true })).toEqual({
      command: 'force-consume-sol-output',
      confirm: true,
    });
